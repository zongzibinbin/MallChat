...
public abstract class AbstractFrequencyControlService<K extends FrequencyControlDTO> {
     ...
     * @throws Throwable
     */
    private <T, V> T executeWithFrequencyControlMap(Map<String, K> frequencyControlMap, SupplierThrowWithParams<T, V> supplier, V param) throws Throwable {
        if (reachRateLimit(frequencyControlMap)) {
            throw new FrequencyControlException(CommonErrorEnum.FREQUENCY_LIMIT);
        }
        try {
     ...
     */
    @SuppressWarnings("unchecked")
    public <T, V> T executeWithFrequencyControlList(List<K> frequencyControlList, SupplierThrowWithParams<T, V> supplier, V param) throws Throwable {
        /*
        ...
        AssertUtil.isFalse(existsFrequencyControlHasNullKey, "限流策略的Key字段不允许出现空值");
        Map<String, FrequencyControlDTO> frequencyControlDTOMap = frequencyControlList.stream().collect(Collectors.groupingBy(FrequencyControlDTO::getKey, Collectors.collectingAndThen(Collectors.toList(), list -> list.get(0))));
        return executeWithFrequencyControlMap((Map<String, K>) frequencyControlDTOMap, supplier, param);
    }
     ...
     * @throws Throwable
     */
    public <T, V> T executeWithFrequencyControl(K frequencyControl, SupplierThrowWithParams<T, V> supplier, V param) throws Throwable {
        return executeWithFrequencyControlList(Collections.singletonList(frequencyControl), supplier, param);
    }
    ...
    @FunctionalInterface
    public interface SupplierThrowWithParams<T, V> {
         ...
         * @return a result
         */
        T get(V paramObj) throws Throwable;
    }
